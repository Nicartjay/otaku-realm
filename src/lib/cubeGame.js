...
// Compat shim: three.js removed Euler.toVector3() around r125.
// This engine (ported from an older CodePen) still calls it in a few
// places to feed Euler components into snapRotation()/setFromVector3().
// Restoring the trivial original implementation keeps the upstream code
// untouched.
if ( typeof THREE.Euler.prototype.toVector3 !== 'function' ) {
  THREE.Euler.prototype.toVector3 = function ( optionalResult ) {
    return optionalResult
      ? optionalResult.set( this.x, this.y, this.z )
...
class World extends Animation {
  ...
  resize() {
    ...
    this.game.dom.ui.style.fontSize = docFontSize + 'px';
...
// touchmove prevention will be applied per-container in initCubeGame
...
class Draggable {
  ...
  getPositionCurrent( event ) {
    ...
    this.position.current.set( dragEvent.clientX - rect.left, dragEvent.clientY - rect.top );
...
class Transition {
  ...
  title( show ) {
    ...
    this.durations.title = title ? ( title.querySelectorAll( 'i' ).length - 1 ) * 50 + ( show ? 800 : 400 ) : ( show ? 800 : 400 );
...
function initRangeElements(container) {
  container.querySelectorAll( 'range' ).forEach( el => {
...
class Range {
  ...
  constructor( name, options ) {
    ...
    this.element = (_gameRoot || document).querySelector( '.range[name="' + name + '"]' );
...
class ThemeEditor {
  ...
  getPieceColor( event ) {
    ...
    const clickPosition = new THREE.Vector2( clickEvent.clientX - rect.left, clickEvent.clientY - rect.top );
...
class IconsConverter {
  ...
  constructor( options ) {
    ...
    if ( options.observe ) {
      ...
      this.observer.observe( this.root || document.documentElement, { childList: true, subtree: true } );
  ...
  convertAllIcons() {
    ...
    (this.root || document).querySelectorAll( this.tagName ).forEach( icon => { this.convertIcon( icon ); } );
  ...
  addStyles() {
    ...
    if ( this.root ) this.root.prepend( style );
...
class Game {
  ...
  constructor(container) {
    ...
    this.dom = {
      ui: container.querySelector( '.ui' ),
      game: container.querySelector( '.ui__game' ),
      back: container.querySelector( '.ui__background' ),
      prefs: container.querySelector( '.ui__prefs' ),
      theme: container.querySelector( '.ui__theme' ),
      stats: container.querySelector( '.ui__stats' ),
      texts: {
        title: container.querySelector( '.text--title' ),
        note: container.querySelector( '.text--note' ),
        timer: container.querySelector( '.text--timer' ),
        complete: container.querySelector( '.text--complete' ),
        best: container.querySelector( '.text--best-time' ),
        theme: container.querySelector( '.text--theme' ),
      },
      buttons: {
        prefs: container.querySelector( '.btn--prefs' ),
        back: container.querySelector( '.btn--back' ),
        stats: container.querySelector( '.btn--stats' ),
        reset: container.querySelector( '.btn--reset' ),
        theme: container.querySelector( '.btn--theme' ),
      },
    };
    ...
    // Always start fresh on page load — don't resume saved games
  ...
  initActions() {
    ...
    this.dom.game.addEventListener( 'click', event => {
      ...
      console.log('[CubeGame] Internal click, state:', this.state, 'activeTransitions:', this.transition.activeTransitions);
  ...
  giveUp() {
    ...
    // Snap cube back to solved state
    this.cube.reset();
    this.cube.init();
...
export function initCubeGame(container) {
  _gameRoot = container;
  window.gameVersion = '0.99.2';
  container.addEventListener('touchmove', e => { e.preventDefault(); }, { passive: false });
  initRangeElements(container);
  const game = new Game(container);
  return {
    game,
    startGame() {
      console.log('[CubeGame] startGame called, state:', game.state, 'activeTransitions:', game.transition.activeTransitions);
    ...
    destroy() {
      animationEngine.remove(game.world);
      game.world.renderer.dispose();
      game.world.renderer.forceContextLoss();
